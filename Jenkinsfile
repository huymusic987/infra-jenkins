pipeline {
    ...
    triggers {
        // This corresponds to "Github hook trigger for GITScm polling"
        githubPush()
    }

    environment {
        // Adds NodeJS to PATH like in your screenshot (NodeJS tool you defined in Jenkins global tools)
        PATH = "${tool 'NodeJS'}/bin:${env.PATH}"
    }

    options {
        timestamps() // you had timestamper enabled in JCasC
    }
    ...
    stages {
        ...
        stage('Build & Push Images') {
            steps {
                ...
                docker login -u huymusic987 -p happyyear27
